use clap::{Parser, Subcommand}; use tracing::{info, error}; use anyhow::Result;

use bleep_core::blockchain; use bleep_wallet_core::wallet_core; use bleep_p2p::p2p_network; use bleep_consensus::consensus; use bleep_governance::governance_engine; use bleep_ai::{ai_assistant, machine_learning}; use bleep_crypto::zkp_verification; use bleep_state::state_manager; use bleep_telemetry::telemetry; use bleep_pat::pat_engine;

#[derive(Parser)] #[command(name = "bleep-cli")] #[command(about = "BLEEP Blockchain CLI", long_about = None)] struct Cli { #[command(subcommand)] command: Commands, }

#[derive(Subcommand)] enum Commands { /// Start the BLEEP blockchain node StartNode,
...
#[derive(Subcommand)] enum WalletCommand { Create, Balance, Import { phrase: String }, Export, }

#[derive(Subcommand)] enum TxCommand { Send { to: String, amount: f64 }, History, }

#[derive(Subcommand)] enum AiCommand { Ask { prompt: String }, Learn { dataset: String }, }

#[derive(Subcommand)] enum GovernanceCommand { Propose { proposal: String }, Vote { proposal_id: u32, yes: bool }, }

#[derive(Subcommand)] enum StateCommand { Snapshot, Restore { snapshot_path: String }, }

#[derive(Subcommand)] enum PatCommand { Predict { input: String }, Autonomy, }

#[derive(Subcommand)] enum BlockCommand { /// Get latest block Latest,
...
#[tokio::main] async fn main() -> Result<()> { tracing_subscriber::fmt::init(); let cli = Cli::parse();
