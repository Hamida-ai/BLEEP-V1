...
use crate::intent::{CrossChainIntent, ZkVerifyIntent};
...
pub fn encode_for_chain(chain: &ChainId, calldata: &[u8]) -> VmResult<Vec<u8>> {
    match chain {
        // EVM chains: calldata is already in ABI format
        ChainId::Ethereum | ChainId::EthereumL2(_) => {
            Ok(calldata.to_vec())
        }

        // Solana: wrap in a Borsh-serialized instruction
        ChainId::Solana => {
        ...
        // Cosmos: wrap in a JSON amino/proto message envelope
        ChainId::CosmosHub => {
            let json = serde_json::json!({
                "type": "wasm/execute",
                "value": {
                    "msg": base64::encode(calldata),
                }
            });
            Ok(serde_json::to_vec(&json).unwrap_or_default())
        }

        // Polkadot: SCALE-encoded extrinsic
        ChainId::Polkadot => {
            // SCALE compact encoding: 2-byte prefix for length < 16384
            ...
            if len < 64 {
                ...
                encoded.push(0x02); // big-integer mode
        ...
        // Near: base64-encoded args in a JSON function call
        ChainId::Near => {
            let json = serde_json::json!({
                "args_base64": base64::encode(calldata),
            });
            Ok(serde_json::to_vec(&json).unwrap_or_default())
        }

        // Sui/Aptos (Move): BCS-encoded transaction
        ChainId::Sui | ChainId::Aptos => {
            // BCS: length prefix as ULEB128
            let len = calldata.len();
        ...
        // BLEEP native: passthrough
        ChainId::Bleep => Ok(calldata.to_vec()),

        // Bitcoin: OP_RETURN data (limited to 80 bytes)
        ChainId::Bitcoin => {
            if calldata.len() > 80 {
...
pub fn decode_from_chain(chain: &ChainId, response: &[u8]) -> VmResult<Vec<u8>> {
    match chain {
        ...
        ChainId::CosmosHub | ChainId::Near => {
            // JSON responses: extract the "result" field
...
/// Status of an outbound cross-chain message.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum MessageStatus {
    /// Message is queued for relay.
    Pending,
    /// Message delivered to destination; awaiting execution.
    Delivered,
    /// Destination executed successfully; result available.
    Confirmed,
    /// Execution failed on destination.
    Failed(String),
    /// Relay timed out.
    Timeout,
}

/// An outbound cross-chain message routed through BLEEP Connect.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CrossChainMessage {
    /// Unique message identifier (SHA-256 of content).
    pub id:                [u8; 32],
    ...
    pub created_at:        u64,
    pub timeout_at:        u64,
    /// ZK proof of source execution (present when require_zk_proof = true).
    pub source_proof:      Option<Vec<u8>>,
    /// Response from destination (present when Confirmed/Failed).
...
impl CrossChainMessage {
    pub fn new(
        ...
        CrossChainMessage {
            ...
            created_at:        now,
            timeout_at:        now + 3600, // 1 hour timeout
            source_proof:      None,
            response:          None,
        }
    }
...
/// Native cross-chain execution bridge — BLEEP Connect integration.
pub struct ConnectBridge {
    gas_model: GasModel,
    /// Supported destination chains and their relayer endpoints.
    supported_chains: HashMap<String, ChainConfig>,
    /// In-flight messages awaiting confirmation.
...
impl ConnectBridge {
    ...
    /// Submit a cross-chain intent for execution.
    /// Returns the message ID and a StateDiff with gas deduction.
    ...
    pub async fn submit(
        ...
        // Encode calldata for destination chain format
        ...
        // Build state diff: deduct relay fee + source gas
        ...
        // Emit a bridge event
        let contract_bytes = [0xBBu8; 32]; // BLEEP Connect contract address
    ...
    /// Query the status of a cross-chain message.
    ...
    /// List all pending messages for a given destination chain.
    pub fn pending_for_chain(&self, chain: &ChainId) -> Vec<[u8; 32]> {
        self.pending.read()
            .iter()
            .filter(|(_, m)| &m.destination_chain == chain
    ...
    /// Return list of supported destination chains.
...
mod tests {
    ...
    fn test_encode_for_solana_length_prefix() {
        ...
        assert_eq!(&encoded[0..4], &[3u8, 0, 0, 0]); // 3 as little-endian u32
    ...
    fn test_sui_bcs_encoding() {
        ...
        // First byte should be 5 (ULEB128 of 5 < 128)
    ...
    fn test_bridge_supported_chains() {
        ...
        assert!(!bridge.is_supported(&ChainId::Bitcoin)); // not in relay set
    }
    ...
    async fn test_submit_to_unsupported_chain_fails() {
        ...
        let sender = [0u8; 32];
        let intent = CrossChainIntent {
            destination_chain: ChainId::Bitcoin, // not supported
            contract:          vec![],
            calldata:          vec![],
            source_gas_limit:  100_000,
        ...
        let result = bridge.submit(&intent, &sender).await;
