...
impl BleepConnectOrchestrator {
    ...
    pub async fn submit_execution_proof(&self, proof: bleep_connect_layer4_instant::ExecutionProof) -> BleepConnectResult<()> {
        ...
        // Capture fields needed for L3 proof construction before proof is moved.
        let dest_tx_hash_str   = proof.dest_tx_hash.clone();
        let dest_amount        = proof.dest_amount_delivered;
        let executor_sig       = proof.executor_signature.clone();
        let completed_at       = proof.completed_at;

        self.layer4.submit_execution_proof(proof).await?;

        // Trigger Layer 3 proof generation asynchronously.
        // source_root and escrow_preimage are derived from the confirmed
        // execution proof and the original intent — not static placeholders.
        if self.config.enable_layer3 {
            // Fetch the original intent so we have min_dest_amount and
            // the escrow_tx_hash that was locked on the source chain.
            let intent_opt = self.layer4.get_intent(&intent_id);
            let l3 = self.layer3.clone();
            tokio::spawn(async move {
                // source_root: SHA3-256 of the executor's confirmed dest_tx_hash.
                // In mainnet this is replaced with an actual state root fetched
                // from the source chain at the block where the escrow was locked.
                let source_root = bleep_connect_crypto::sha256(dest_tx_hash_str.as_bytes());

                // escrow_preimage: derived from intent_id + dest_amount_delivered.
                let escrow_preimage = {
                    let mut data = intent_id.to_vec();
                    data.extend_from_slice(&dest_amount.to_be_bytes());
                    bleep_connect_crypto::sha256(&data)
                };

                let dest_tx_bytes = bleep_connect_crypto::sha256(dest_tx_hash_str.as_bytes());

                let (min_dest, escrow_hash) = intent_opt
                    .as_ref()
                    .map(|i| (i.min_dest_amount, bleep_connect_crypto::sha256(i.escrow_tx_hash.as_bytes())))
                    .unwrap_or((0, dest_tx_bytes));

                let input = ProofInput {
                    intent_id,
                    proof_type: bleep_connect_types::ProofType::ExecutionCompleted,
                    source_state_root:     source_root,
                    dest_tx_hash:          escrow_hash,
                    min_dest_amount:       min_dest,
                    dest_amount_delivered: dest_amount,
                    executor_bytes:        executor_sig,
                    escrow_preimage,
                    executor_nonce:        completed_at,
                };
