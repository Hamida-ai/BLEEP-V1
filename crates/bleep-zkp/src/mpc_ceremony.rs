...
impl MPCCeremony {
    ...
    pub fn contribute(&mut self, participant: Participant) -> Result<TranscriptEntry, CeremonyError> {
        ...
        // Compute running hash: SHA3-256(current_hash || contribution_hash)
        // Both inputs are concatenated before hashing so the transcript hash
        // is cryptographically sound — each contribution irreversibly mixes
        // into the running state.
        use sha3::{Sha3_256, Digest};
        let new_hash: [u8; 32] = {
            let mut h = Sha3_256::new();
            h.update(&self.current_hash);
            h.update(&participant.contribution_hash);
            h.finalize().into()
        };
    ...
    pub fn verify_transcript(&self) -> VerificationResult {
        ...
        for (i, entry) in self.transcript.iter().enumerate() {
            ...
            // Re-derive the running hash using the same SHA3-256 as contribute()
            use sha3::{Sha3_256, Digest};
            let expected: [u8; 32] = {
                let mut h = Sha3_256::new();
                h.update(&hash);
